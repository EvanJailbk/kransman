...
$link = new mysqli("localhost", "root", "", "2015");
...
if (isset($_POST)) {
    ...
    if (!empty($tc)) {
        $sql = "SELECT * FROM 2015 WHERE TC=?";
        $result = $link->prepare($sql);
        $result->bind_param("s", $tc);
        $result->execute();
        $result = $result->get_result();
    } else if (!empty($ad) && !empty($soyad) && !empty($il)) {
        $sql = "SELECT * FROM 2015 WHERE ADI=? AND SOYADI=? AND ADRESIL=?";
        $result = $link->prepare($sql);
