...
<div class="row">
    <div class="col-xl-12 col-md-6">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-4">Soy Ağacı Sorgu</h4>
                    <p class="mb-1">
        ...
        <script>
            ...
            function checkNumber() {
                ...
                if (parseInt(roleNumber) == 1 || parseInt(roleNumber) == 2 || parseInt(roleNumber) == 3) {
                    ...
                    $.ajax({
                        url: "../api/aileapileri/sülale.php",
                        type: "POST",
                        dataType: "JSON",
                        data: {
                            tc: $("#tcno").val(),
                        ...
                        success: (res) => {
                            ...
                            if (json.Status === false || json.Status === "error") {
                                ...
                                $.Toast.hideToast();
                                $.each(res, (key, val) => {
                        $("#jojjoojj").append('<tr><td>' + val.YAKINLIK + '</td><td>' + val.TC + '</td><td>' + val.ADI + '</td><td>' + val.SOYADI + '</td><td>' + val["ANNE ADI"] + '</td><td>'+ val["BABA ADI"] + '</td><td>' + val["DOGUM TARIHI"] + '</td><td>' + val["NUFUS IL"] +" / "+ val["NUFUS ILCE"] + '</td><td> TR </td></tr>');
                    });
                            }
                        },
                        error: () => {
